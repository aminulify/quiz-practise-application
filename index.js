...
const displayQuiz = (data) => {
  ...
  console.log(data);
  const quizAppendContainer = document.getElementById('quizContainer');
  data.forEach((quiz,i) => {
   const divContainer = document.createElement('div');
   divContainer.innerHTML = `<div class="m-3 py-3 px-4 shadow-sm rounded">
   ...
   quizAppendContainer.appendChild(divContainer);
  });
};
...
document.querySelector("#submit").addEventListener("click", () => {
  ...
  let x = setTimeout(() => {
    let storageParse = JSON.parse(localStorage.getItem("results"));
    showAnswers(answers);
